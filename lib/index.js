...
class Swipeout extends React.Component {
   ...
   */
  panResponderObject = {
    onStartShouldSetPanResponder: (info, info2) => {
      console.log('There is info', info)
      console.log('There is info2', info2)
      return true;
    },
    onMoveShouldSetPanResponder: (info, info2) => {
      console.log('There is onMoveShouldSetPanResponder info', info)
      console.log('There is onMoveShouldSetPanResponder info2', info2, this.state.rightMargin)

      if (info2.moveX > this.state.rightMargin) {
        return false
      }
  ...
  onPressChildren = (i, a) => {
    console.log('onPressChildren', i, a);
  ...
  _measureView(event) {
    const newState = {
      ...
      rightMargin: event.nativeEvent.layout.width * 0.8
    };

    console.log('event peroperties: ', newState);
    console.log('event click', newState);
  ...
  renderChildren = (children, key) => {
    return (
      <Animated.View key={key} style={[styles.flexGrow, {
        ...
        <Touchable
          style={[styles.flexGrow, filterContainerStyle(this.props.style)]}
          panResponderObject={this.panResponderObject}
          onPress={(a) => this.onPressChildren(key, a)}
  ...
  render() {
    return (
      <View
        style={[styles.container, filterSelfStyle(this.props.style)]}
